...
def main(args):
    ...
    wandb.init(project=f"GF-PSO-{args.net}-{args.dataset}", config={
        "W_init": args.w_init,
        "steps": args.steps,
    ...
    solver = PSO(population_size=popsize, solution=np.zeros(D))  
    ...
    state_wandb = {}
    pbar = tqdm(range(1, args.steps + 1), desc="Training with PSO", disable=not args.verbose)
    for step in pbar:
        ...
        if step % args.gd_interval == 0 or step == 1:
            ...
            if val_accuracy_after_sgd >= val_accuracy:
                ...
                solver = PSO(population_size=popsize, solution=x0)
    ...
    save_model(model, f"GF-PSO-{args.net}-{args.dataset}-LR{args.lr_init}-W_init{args.w_init}-batch_size{args.batch_size}-steps{args.steps}-warm{args.warm}", wandb)
...
def parse_args():
    ...
    """
    parser = argparse.ArgumentParser(description='Train ResNet on CIFAR using PSO')
    ...
    parser.add_argument('--steps', type=int, default=200,
                      help='Number of steps for PSO (default: 200)')
    parser.add_argument('--popsize', type=int, default=50,
                      help='Population size for PSO (default: 50)')
    ...
    parser.add_argument('--eval_interval', type=int, default=10,
                      help='Evaluation interval for PSO (default: 10)')
