...
def train_epoch(es, es_params, es_state, key, ws, train_loader, val_loader, epoch, num_iterations, learning_rate, device, args):
    ...
    for count_batch in range(1, num_batches+1):
        ...
        # Log to wandb
        if (count_batch - 1) % period == 0 or count_batch == num_batches or count_batch == 1:
...
def main(args):
    ...
    if not args.disable_wandb:
        ...
        init_wandb(
            project_name=f"{args.wandb_project}-{args.dataset}-{args.arch}-v2", 
            run_name=run_name,
            config=config,
    ...
    while epoch <=  args.epochs:
        ...
        theta_0 = theta_t # Hard update on theta_0
